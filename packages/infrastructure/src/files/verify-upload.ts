...
export function verifyUpload(
  ...
  if (input.bytes.byteLength > maxBytes)
    throw uploadVerificationError("UPLOAD_TOO_LARGE", "Upload exceeds the configured byte limit");
  if (input.bytes.byteLength !== input.expectedBytes)
    throw uploadVerificationError(
      "SIZE_MISMATCH",
      "Upload byte count does not match the declared size",
    );
  ...
  if (dimensions && dimensions.width * dimensions.height > maxPixels)
    throw uploadVerificationError(
      "IMAGE_PIXEL_LIMIT",
      "Image dimensions exceed the decompression bomb limit",
    );
  return {
    checksumSha256,
    ...
    ...(dimensions?.alpha === undefined ? {} : { alpha: dimensions.alpha }),
    metadataJson: {
      detectedMimeType: input.mimeType,
      width: dimensions?.width ?? null,
      height: dimensions?.height ?? null,
      alpha: dimensions?.alpha ?? null,
    },
  };
}
...
export function createUploadVerifier(
      ...
      return verifyUpload(
        {
          ...
          expectedBytes: input.session.bytes,
...
export function uploadSessionToVerificationInput(
  session: { readonly bytes: number; readonly mimeType: string },
  bytes: Uint8Array,
