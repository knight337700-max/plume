...
describe("staging migration runner contract", () => {
  ...
  it("reports pending and applied migrations without mutating", () => {
    ...
    const applied = createMigrationPlan(definitions, new Map([["0001_initial", { checksum: "checksum-1", appliedAt: "now" }]]));
  ...
  it("rejects destructive SQL and unconfirmed staging apply", () => {
    ...
    expect(() => assertStagingTarget({ appEnv: "production", databaseUrl: "postgresql://db/plume" })).toThrow(/APP_ENV/);
