...
describe("canonical media channel catalog", () => {
  it("contains four unique top-level channels and keeps Bizboard below Kakao Moment", async () => {
    expect(CANONICAL_CHANNELS.map((channel) => channel.id)).toEqual([
      "NAVER_GFA",
      "KAKAO_MOMENT",
      "META",
      "GOOGLE_ADS",
    ...
    expect(
      (await createCanonicalCatalogRepository().listChannels()).map((channel) => channel.code),
    ).toEqual(["GOOGLE_ADS", "KAKAO_MOMENT", "META", "NAVER_GFA"]);
  });

  it("keeps approved format identifiers stable", () => {
    expect(new Set(APPROVED_FORMAT_PROFILES.map((format) => format.id)).size).toBe(
