...
export function createGenerationUseCases(repositories: CampaignRepositories): GenerationUseCases {
  return {
    async create(input) {
      ...
      if (!briefVersionId || !brief?.currentVersionId || brief.currentVersionId !== briefVersionId) { const error = new Error("Generation must use the current brief version"); Object.assign(error, { code: "BRIEF_VERSION_STALE", statusCode: 409 }); throw error; }
      ...
      const aggregate = await repositories.createGenerationAggregate({ workspaceId: input.workspaceId, campaignId: input.campaignId, briefVersionId, estimatedItemCount: items.length }, items);
      return aggregate;
    },
