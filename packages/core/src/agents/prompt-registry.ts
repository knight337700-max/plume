...
const PROMPTS: readonly PromptDefinition[] = Object.freeze(
  ...
  ].map(([promptId, agentCode, systemTemplate, outputSchemaId, modelPolicyId, contentHash]) => ({
    promptId,
    agentCode: agentCode as AgentCode,
    version: "1.0.0",
    systemTemplate,
    inputTemplate: "Use only the scoped workspace context and return the registered output schema.",
    outputSchemaId,
    safetyPolicyId:
