...
interface Options { readonly uploads?: UploadUseCases; readonly assets?: AssetUseCases; readonly repositories?: AssetRepositories }
...
export const assetRoutesGroup: FastifyPluginAsync<Options> = async (app, options) => {
  const assets = options.assets ?? createAssetUseCases(options.repositories ?? createInMemoryAssetRepositories());
