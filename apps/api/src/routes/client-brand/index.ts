...
export interface ClientBrandRouteOptions {
  readonly repositories?: ClientBrandRepositories;
}

export const clientBrandRoutes: FastifyPluginAsync<ClientBrandRouteOptions> = async (
  app,
  options,
) => {
  const repositories = options.repositories ?? createInMemoryClientBrandRepositories();
