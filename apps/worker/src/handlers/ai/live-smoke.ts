...
export function createLiveSmokeHandler(gateway: AgentProviderGateway): RuntimeJobHandler {
  let requestCount = 0;
  return async (job: Job<unknown>) => {
    ...
    const workspaceId = payload.workspaceId ?? "00000000-0000-4000-8000-0000000002c0";
    ...
    const orchestrator = createAgentOrchestrator({
      gateway: {
        async execute(request) {
          if (requestCount >= requestBudget) throw new Error("LIVE_SMOKE_REQUEST_BUDGET_REACHED");
          requestCount += 1;
